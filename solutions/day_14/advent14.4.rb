...
STORE = PStore.new("shared_storage.pstore")
...
def memoized_hashes(key)
  STORE.transaction { STORE[:hashes] ||= {}; STORE[:hashes][key] }
end

def set_memoized_hash(key, value)
  STORE.transaction { STORE[:hashes] ||= {}; STORE[:hashes][key] = value }
end

def keys
  STORE.transaction { STORE[:keys] ||= [] }
end

def add_key(key)
  keys
  STORE.transaction { STORE[:keys] << key }
end
...
def generate_hash(hash)
  ...
  set_memoized_hash([start_hash, iterations], hash)
  hash
end
...
def test_key(index)
  ...
  if char = contains_run?(hash_1, 3)
     Parallel.each((1..1000).to_a, in_processes: nprocessors) do |j|
      hash_2 = generate_hash("#{salt}#{index+j}")
      if contains_run?(hash_2, 5, char)
        add_key key: hash_1, index: index
        debug "\n"
        debug "#{keys.length+1}: #{index}\n"
        raise Parallel::Kill
      end
    end
  end
...
def find_keys
  begin
    ...
    File.delete(STORE.path) rescue nil
  end
end
